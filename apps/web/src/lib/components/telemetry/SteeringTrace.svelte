<script lang="ts">
	...
	let data = $derived.by(() => {
		...
		for (const key of intersection) {
			...
			if (!mainData || mainData.steer === undefined) continue;

			const y = mainData.steer;

			if (comparisonTelemetryData) {
				const compData = comparisonTelemetryData[key];
				if (!compData || compData.steer === undefined) continue;

				const y1 = compData.steer;
	...
	let yDomain: [number, number] = $state([-1, 1]);
	...
	const color = (d: DataRecord | null, i: number) => ["#fcba03", "#976f02"][i];
...
<Chart title="Steering Trace">
	...
	<div class="container" bind:this={container}>
		<VisXYContainer {xDomain} {yDomain}>
			...
			/>
			<VisAxis type="y" label="Steering Lock (normalised)" />
			<VisTooltip />
