<script lang="ts">
	...
	let data = $derived.by(() => {
		...
		for (const key of intersection) {
			...
			if (!mainData || mainData.throttle === undefined) continue;

			const y = mainData.throttle * 100;

			if (comparisonTelemetryData) {
				const compData = comparisonTelemetryData[key];
				if (!compData || compData.throttle === undefined) continue;

				const y1 = compData.throttle * 100;
	...
	const color = (d: DataRecord | null, i: number) => ["#3db33b", "#216020"][i];
