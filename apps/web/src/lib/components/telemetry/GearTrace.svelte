<script lang="ts">
	...
	let data = $derived.by(() => {
		...
		for (const key of intersection) {
			...
			if (!mainData || mainData.gear === undefined) continue;

			const y = mainData.gear;

			if (comparisonTelemetryData) {
				const compData = comparisonTelemetryData[key];
				if (!compData || compData.gear === undefined) continue;

				const y1 = compData.gear;
	...
	let yDomain: [number, number] = $state([0, 8]);
...
<Chart title="Gear Trace">
	...
	<div class="container" bind:this={container}>
		<VisXYContainer {xDomain} {yDomain}>
			...
			/>
			<VisAxis type="y" numTicks={8} label="Gear" />
			<VisTooltip />
